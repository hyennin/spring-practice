...
public class MyEntity {
    ...
    @Id
    // @GeneratedValue 어노테이션은 필드값(주로 Primary Key)을 자동으로 생성하는 방식을 설정하기 위해서 사용함
    // (데이터베이스 프로그램마다 키를 자동 생성하는 방식(auto increment)이 지원되지 않거나 혹은 다른 방식으로 키 생성(ex: 오라클의 경우 시퀀스 객체를 사용)을 하므로 키 자동 생성 방식을 직접 정의하기 위해서 사용)
    ...
    // @Column(columnDefinition="ENUM('HELLO', 'WORLD')") @Enumerated(EnumType.STRING)
